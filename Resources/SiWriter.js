			...
			var symbol	=	0; // first click happens to hit K6
			var Key = 0;
			var Caps = 0;
			var Start=1;
			var delKey=0;
			...
			var Caps_lock=false;	
			var LastLetter="";
			var PPPPPPreviousLetter="";
			var PPPPPreviousLetter="";
			var PPPPreviousLetter="";
			var PPPreviousLetter="";
			var PPreviousLetter="";	
			var PreviousLetter="";	
			var s="!?!";
			var checkletters="!?!";
			...
			function setupKeys(){  // detects and acts on 'key' press.
				...
				did("k5").onTouchUp = function(info){
					...
					$("#k5").addClass("CapsKeyUp");
								
					if (Caps==32){
					Caps_lock=!Caps_lock;
						if(Caps_lock==false){
							$("#k5").removeClass("CapsKeyUp");Caps=0;
						};
					}else{
						Caps=32;//Caps key
					};
						...
						$("#k5").addClass("CapsKeyUpL");
					}else{$("#k5").removeClass("CapsKeyUpL");}
				...
				did("k6").onTouchUp = function(info){
					...
					$("#k6").addClass("NumKeyUp");
					
					if (Nums==64){
					Num_lock=!Num_lock;
						if(Num_lock==false){
							$("#k6").removeClass("NumKeyUp");Nums=0;
						};
					}else{
					...
					if ((Nums==64)&&(Num_lock==true)){
						$("#k6").addClass("NumKeyUpL");
					}else{$("#k6").removeClass("NumKeyUpL");}
		...
		setupKeys();
			function focusTFC(el){ // gives focus something to change content display - ie key tops
				...
				$(".chordKey").stop().css({opacity: 0.95}).fadeIn(250, function(){
					keyboardShown = true;
					processKeyDown();
				});
				...
				$(".chordKeyx").stop().css({opacity: 0.95}).fadeIn(250, function(){
					keyboardShown = true;
					processKeyDown();
				});
...
if(s != "_" && s != "..."){ //if buttons are released and they have something to say - send it...
		...
		/*setWindshield(PPPPPPreviousLetter+PPPPPreviousLetter+PPPPreviousLetter+PPPreviousLetter+PPreviousLetter+PreviousLetter+LastLetter+s);	
		PPPPPPreviousLetter=PPPPPreviousLetter;
		PPPPPreviousLetter=PPPPreviousLetter;
		PPPPreviousLetter=PPPreviousLetter;		
		PPPreviousLetter=PPreviousLetter;
		PPreviousLetter=PreviousLetter;		
		PreviousLetter=LastLetter;	
		LastLetter=s;
   */
 
 
		Ti.App.fireEvent('webviewEvent', {text: s+"_"});//carat symbol added
		...
		////////last key action held or reset here/////////
				...
				if(symbol==1) {symbol=0;}; 
					$("#k5").removeClass("CapsKeyUp");
				    $("#k6").removeClass("NumKeyUp");
								
			if(Num_lock==true) {Nums=64;$("#k6").addClass("NumKeyUp");}
			if(Caps_lock==true){Caps=32;$("#k5").addClass("CapsKeyUp");}
...
//Ti.API.info('**********************************//setWindshield='+setWindshield);
 ...
 if (Start==1){
	...
	//Ti.API.info("s="+s); 
			...
			function sortIndexedNumbers(a,b){
				return a[1] - b[1];
			}
